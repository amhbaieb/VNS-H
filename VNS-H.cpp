...
#define ITERMAX 500
	...
	int Nv,Np;
	int *IDp,*IDv;
	double time_best,time_tot;
	int iter=0,iter_tot=0,iter_best=0;;
...
double calcul_cost(){
	...
	double cccc=0;
	for(j=0;j<Np;j++)
		if(n_vm_par_pm[j]>=1) cccc+=COSTp(j);
	return cccc;
}
bool verif_cost(){
	double vc=0;
	for(j=0;j<Np;j++){
		...
		if(vc!=n_vm_par_pm[j]) return false;
	}
	return true;
}
...
double res_DISK_pm2[24];
bool test_move_PM_PM(int pm1,int pm2){

	bool pos=true;
	double res_CPU_pm2=res_CPU_phy[pm2];
	double res_MEM_pm2=res_MEM_phy[pm2];	
	for(int p=0;p<nDISKp(pm2);p++){
		res_DISK_pm2[p]=res_DISK_phy[pm2][p];
	...
	res_CPU_phy[pm2]=res_CPU_pm2;
	res_MEM_phy[pm2]=res_MEM_pm2;
	for(int p=0;p<nDISKp(pm2);p++){
		res_DISK_phy[pm2][p]=res_DISK_pm2[p];
	}
	...
	for(int i=0;i<Nv;i++){
		...
		for(int j=0;j<Np;j++){
			...
			if((test_embedd_PM_VM(listePm[j],listeVm[i])!=nullptr)&&(listePm[j]!=X[listeVm[i]])){
				...
				if(a11<a1){
					//printf("vm%d-pm%d=>%lf,%lf\n",listeVm[i],listePm[j],a1,a11);
...
void recherche_local_N2(){
	...
	for(int i=0;i<Nv;i++){//listeVm i
		index=0;
		double a1=calcul_cos_angle(X[listeVm[i]]);
		for(int j=0;j<Nv;j++){//listeVm j
...
void recherche_local_N3(){
	...
	for(int l=0;l<Np;l++)
		for(k=0;k<Nv;k++)
			if(X[k]==listePm[l]){listeVm[cpt]=k;cpt++;}
	...
	double c1;
	int pm1,vm1;
	for(int i=0;i<0.2*Nv;i++){//listeVm[i]
		
		index=0;
		for(int j=Np-1;j>=0;j--){//listePm[j] destination (target)
...
void recherche_local_N4(){
	...
	for(int i=cpt-1;i>=0;i--){//listePm[i]]
...
void freeTabs(){
	...
	free(PM);
	free(b);free(disk);free(used_phys_disk);free(R);
	free(yy1);free(yy2);
...
bool InitialSolution(){
	...
	for(j=0;j<Nv;j++){
		...
		//try to embed vm on an unused pm
		if(embed==false){
		...
		if(embed==false){ 
			printf("\nNO embedd VM%d (%d) diskv:%d!", VM[j],j,nDISKv(VM[j]));
			break;
		}
...
void main()
{
	...
	while(!fl.eof())
	{
		...
		for(int run=1;run<=5;run++){
			...
			ofstream ff("res.xls", ios::out | ios::app );
			if(!ff) printf("Error opening Data_center.txt file...\n");
	
			init_solution();//initialization of data structures for the initial solution
			printf("begin resolution...\n");
			bool embed= InitialSolution();	//create initial solution
		
			if(embed!=true)
			{
				ff<<endl<<instance<<"\t"<<"no embedd";				
			}
			else
			{
				
				ff<<endl<<instance<<"\t"<<cost<<"\t";
				t1 = clock();
	
				int n=1,sh=1;;
				init_recherche(); //initialisation des structure de donnee pour la recherche locale
				srand(1000*run);
				//srand(1977);
				update();//mise a jour de la fonction objectif et enregistrement de la meuilleure solution rencontree

				iter_tot=0;iter=0;iter_best=0;
				do
				{
					iter_tot++;
					iter++;	
					...
					{
						update();
						time_best = (double)(clock() - t1)/(double)CLK_TCK ;
						if(iter_tot>0) 
							c[currentshaking]++;
						iter_best=iter_tot;
						iter=0;
				...
				time_tot = (double)(clock() - t1)/(double)CLK_TCK ;
				ff<<cost<<"\t"<<time_best<<"\t"<<iter_best<<"\t"<<time_tot<<"\t"<<iter_tot;
				printf("%d\t%s\t%.0lf\t\t%.2lf\t%.2lf\t%d\t%d\n",++tt,instance.c_str(),calcul_cost(),time_best,time_tot,iter_best,iter_tot);
				
				
				if (!verif_cost()) system("pause");
